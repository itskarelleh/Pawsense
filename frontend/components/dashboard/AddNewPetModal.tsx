"use client"
import { Plus, Female, Male, QuestionMark } from 'iconoir-react';
...
import { addNewPet, getPetById } from '@/actions';
    ...
    <div>
      ...
      <Modal title="Add New Pet" isOpen={isOpen} openModal={openModal} closeModal={closeModal}>
        ...
        <Formik initialValues={initialValues} onSubmit={async (values, actions) => {
          ...
          const avatar = await uploadImage(values);
          console.log("Uploading avatar to cloudinary");
          console.log(avatar.public_id);


          const modifiedBody = {...values, avatar: avatar.public_id}
...
function PetAvatarField() {
  ...
  return (
    <div className="flex flex-col items-center justify-center relative mb-8">
      <div className="relative cursor-pointer w-24 h-24 rounded-full overflow-hidden bg-neutral-400">
  ...
  return (
    <RadioGroup value={plan} onChange={setPlan} className="w-full">
      <RadioGroup.Label>Sex</RadioGroup.Label>
        ...
        {({ checked }) => (
          <label className={`text-center flex flex-col items-center justify-center ${checked ? 'bg-pink-200' : ''}`}>
            <Female />
            Female
          </label>
        )}
      ...
      <RadioGroup.Option value="M">
        {({ checked }) => (
          <label className={`text-center flex flex-col items-center justify-center ${checked ? 'bg-blue-200' : ''}`}>
            <Male />
            Male
          </label>
        )}
      ...
      <RadioGroup.Option value="I">
        {({ checked }) => (
          <label className={`relative text-center flex flex-col items-center justify-center ${checked ? 'bg-purple-200' : ''}`}>
            <Intersex />
            Intersex
          </label>
        )}
      ...
      <RadioGroup.Option value="?">
        {({ checked }) => (
          <label className={`text-center flex flex-col items-center justify-center ${checked ? 'bg-purple-200' : ''}`}>
            <QuestionMark />
            Unknown
          </label>
        )}
